...
void MainWindow::updateClassProperties()
{
    if (current_db_class == NULL)
        return;
    if (CLLCListWidget->highlightedRow() < 0)
        return;
...
void MainWindow::addSession()
{
    ...
    for (int i = 0; i < SVLSListWidget->count(); ++i) {
        if (sessions_list.contains(SVLSListWidget->item(i)->data(Qt::UserRole).toDateTime()))
            continue;
...
{
    if (CLSSResultsTableWidget->currentRow() < 0)
        return;
